<?php
    ...
    */
    function get_message_rows() 
    {
        ...
        $stmt->execute();
        $stmt->bind_result($username, $message);
    
        // Fetch all rows and store in the results array
        while ($stmt->fetch()) {
            $results[] = array('username' => $username, 'message' => $message);
        }
    ...
    */
    function is_valid_image($image_path) 
    {
        ...
        // Use getimagesize to check if the file is a valid image
        $image_info = getimagesize($image_path);

        // Check if getimagesize returns false or the mime type is not an image
    ...
    */
    function add_photo_path_to_user($username, $file_userphoto) 
    {
        ...
        // Use mysqli_real_escape_string to sanitize input and prevent SQL injection
        $username = mysqli_real_escape_string($conn, $username);
        $file_userphoto = mysqli_real_escape_string($conn, $file_userphoto);
    ...
    */
    function get_photo_path_for_user($username) 
    {
        ...
        // Use prepared statements to prevent SQL injection
        $stmt = $conn->prepare("SELECT file_userphoto FROM users WHERE username = ?");
        ...
        // Return the path or null if it's not set
    ...
    */
    ...
    {
        // Validate user input to ensure it is a valid username and memo name
        if (!preg_match('/^[a-zA-Z0-9_]+$/', $username) || !preg_match('/^[a-zA-Z0-9_]+$/', $memoname)) {
            return "Invalid input!";
        }

        // Construct the path to the memo file
        $memo_path = "memos/{$username}/{$memoname}.txt";

        // Check if the memo file exists
        if (!file_exists($memo_path)) {
            return "No such file!";
        }

        // Read and return the content of the memo file
    ...
    */
    ...
    {
        // Validate user input to ensure it is a valid language code
